...
export function UserRequestDetailModal({
  ...
  return (
    <div
      aria-modal="true"
      className="fixed inset-0 z-[190] grid place-items-center bg-slate-950/55 p-3 sn-dialog-overlay-in"
      role="dialog"
    >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {state.status === "loading" ? (
            ...
            <div className="grid min-h-48 place-items-center rounded-2xl bg-slate-50 p-6 text-center">
              <p className="text-sm font-medium text-slate-700">{state.error}</p>
            </div>
          ) : (
