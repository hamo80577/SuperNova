...
This prompt is for the branch that already contains Phase 8A backend role-scoped report API work.

## Required Branch Workflow

Start from the Phase 8A branch, not from `main`, unless Phase 8A has already been merged.

Recommended flow while Phase 8A is still open:

```bash
git checkout feature/attendance-role-scoped-reports
git pull
git checkout -b feature/attendance-role-scoped-report-ui
```

If `feature/attendance-role-scoped-report-ui` already exists locally, switch to it and rebase/merge the latest `feature/attendance-role-scoped-reports` before editing.
...
- Source chain/source branch filters are still imported source labels and only filter inside the backend-scoped data.
...
- Keep Admin/Super Admin navigation unchanged except for small consistency fixes if needed.
...
- `apps/web/components/dashboard/dashboard-shell` or the current dashboard frame implementation
...
3. Example variant is acceptable:
...
- Source chain/source branch filters still work inside the allowed scope.
